    ...
    <div>
	  ...
	  <li><a></a></li>
	   ...
	   <li><a data-toggle="modal" href="#myModal">New Message</a></li>
        ...
        <li class="active"><a href=<?php echo base_url('MsgPortal/replied/'); ?>>Replied</a></li>
...
<?php echo $this->session->flashdata('msgrep'); ?>
...
<table class="table table-striped">
	...
	$flag=0;
   foreach($this->session->userdata('replied') as $show)
   {
	  ...
	  {
echo "<tr>"."<td>".$show->Sub."</td>"."<td><a href='".base_url('MsgPortal/convo/'.$show->Person.'/R')."'>".$show->Msg."</a></td>"."<td>".$show->Person."</td>"."<td>".$show->Date_Time."</td>"."<td><a href='".base_url('MsgPortal/logdel/'.$show->Person)."'><button type='button' data-toggle='tooltip' data-placement='right' title='This will move your conversation to Deleted section. If you have any conversation with that person later then that shall not be displayed unless you restore it back ' id='delbutt'   class='btn btn-default'>"."Move To Deleted"."</button></td>"."</tr>";
...
$("#sea").keyup(function(){
  ...
  $.ajax({
		url : '<?php echo base_url('MsgPortal/searconvorep/'); ?>',
		type : 'POST',
		data : {
			check:chatter.value,
			},
